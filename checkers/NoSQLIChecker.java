...
public class NoSQLIChecker {
    ...
    private void processJsonNode(Object node, String path, String url, HttpRequest originalRequest, boolean bypassDelay) {
        ...
        if (node instanceof JSONObject) {
            ...
            testJsonPath(newPath, url, originalRequest, bypassDelay);
    ...
    private void testJsonPath(String path, String url, HttpRequest originalRequest, boolean bypassDelay) {
        ...
        if (setJsonValue(modifiedJson, path, isEmptyArrayTest ? "" : value)) {
            ...
            // Step 2: Send $eq request (first try raw JSON)
            ...
            // Skip $ne for arrays
            if (nePayload == null) {
                core.logger.log("JSON", "Skipping $ne test for array JSON parameter: " + path);
                return;
            }

            // Step 3: Send $ne request (use stringified payload if $eq required it)
            HttpRequestResponse neResp = null;
            int neStatus = 0;
            int neLength = 0;
            boolean usedStringifiedNePayload = false;

            if (usedStringifiedEqPayload) {
                // If $eq used a stringified payload, use it for $ne as well
                ...
                if (setJsonValue(modifiedJson, path, stringifiedNePayload)) {
                    ...
                    core.logger.log("JSON", "Sending $ne JSON payload for: " + path + ", Payload: " + stringifiedNePayload + " (stringified due to $eq behavior)");
            ...
            if (baseLength == eqLength) {
                // Step 6: Check if $ne response differs from $eq
                if (neLength != eqLength) {
                    core.logger.log("JSON", "[VULNERABLE] Possible NoSQL Injection found for JSON parameter: " + path + ", $ne payload: " + (usedStringifiedNePayload ? nePayload.replace("\"", "\\\"") : nePayload));
                    Annotations annotations = Annotations.annotations()
                            .withHighlightColor(HighlightColor.RED)
                    ...
                    core.logger.log("JSON", "No vulnerability detected for JSON parameter: " + path + " (baseLength == eqLength, and neLength == eqLength)");
                }
            } else {
                // Step 7: Final check
                if (neLength != eqLength) {
                    ...
                    core.logger.log("JSON", "No vulnerability detected for JSON parameter: " + path + " (baseLength != eqLength, but neLength == eqLength)");
                }
            }
        }
    }
